...
class Piece {
  ...
  drawAvailableMoves() {
    ...
    this.availableMoves.forEach((indexPair) => {
      if (!(indexPair[0] == this.x && indexPair[1] == this.y)) {
        context.fillRect(
          (indexPair[0] * BOARD_WIDTH) / 8,
          (indexPair[1] * BOARD_WIDTH) / 8,
          BOARD_WIDTH / 8,
          BOARD_WIDTH / 8
        );
      }
  ...
  draw(imageSource) {
    if (this.isClicked) {
      var boardWidth = window.innerHeight * 0.8;
      ...
      context.drawImage(
        drawing,
        (xVal * BOARD_WIDTH) / 8 - (BOARD_WIDTH * 3) / 64,
        (yVal * BOARD_WIDTH) / 8 - (BOARD_WIDTH * 3) / 64,
        (BOARD_WIDTH * 3) / 32,
        (BOARD_WIDTH * 3) / 32
      );
    } else {
      ...
      context.drawImage(
        drawing,
        (this.x * BOARD_WIDTH) / 8 + BOARD_WIDTH / 64,
        (this.y * BOARD_WIDTH) / 8 + BOARD_WIDTH / 64,
        (BOARD_WIDTH * 3) / 32,
        (BOARD_WIDTH * 3) / 32
      );
    }
    ...
    for (let i = 0; i < squares1.length; i++) {
      if (
        ...
        if (squares[squares1[i][0]][squares1[i][1]].isOccupied() && !blocked) {
          ...
          blocked = true;
          if (
            squares[squares1[i][0]][squares1[i][1]].piece.type === 'king' &&
...
class King extends Piece {
  ...
  findAvailableMoves() {
    ...
    squaresToCheck.forEach((square) => {
      if (square[0] > -1 && square[0] < 8 && square[1] > -1 && square[1] < 8) {
        ...
        if (
          this.noThreat(square, this.team) &&
          ((mySquare.isOccupied() && mySquare.piece.team != this.team) ||
            !mySquare.isOccupied())
        ) {
  ...
  noThreat(square, team) {
    return (
      !this.pawnThreat(square, team) &&
      !this.rookThreat(square, team) &&
      !this.knightThreat(square, team) &&
      !this.bishopThreat(square, team) &&
      !this.queenThreat(square, team) &&
      !this.kingThreat(square, team)
    );
  }
  pawnThreat(square, team) {
    if (team === -1) {
      ...
      if (square[0] - 1 > -1 && square[1] + 1 < 8) {
        ...
        if (pawnSquare.piece != null) {
          if (
            pawnSquare.piece.type === 'pawn' &&
            pawnSquare.piece.team === -1
          ) {
            return true;
          }
        }
  ...
  kingThreat(square, team) {
    ...
    for (let i = 0; i < squaresToCheck.length; i++) {
      if (
        squaresToCheck[i][0] > -1 &&
        squaresToCheck[i][0] < 8 &&
        squaresToCheck[i][1] > -1 &&
        squaresToCheck[i][1] < 8
      ) {
    ...
    pieces.forEach((piece) => {
      if (!threat && piece.type === pieceType && piece.team != team) {
        ...
        coveredSquares.forEach((coveredSquare) => {
          if (
            coveredSquare[0] === square[0] &&
            coveredSquare[1] === square[1]
          ) {
            threat = true;
          }
        });
...
class Pawn extends Piece {
  ...
  findAvailableAttacks() {
    ...
    if (
      attackSquare1 != null &&
      attackSquare1.piece != null &&
      attackSquare1.piece.team != this.team
    ) {
      this.availableMoves.push(attack1);
    }
    if (
      attackSquare2 != null &&
      attackSquare2.piece != null &&
      attackSquare2.piece.team != this.team
    ) {
  ...
  findAvailableMoves() {
    ...
    if (
      this.y > 0 &&
      this.y < 7 &&
      squares[this.x][this.y + this.team].piece == null
    ) {
      this.availableMoves.push([this.x, this.y + this.team]);
      if (
        !this.hasMoved &&
        squares[this.x][this.y + 2 * this.team].piece == null
      ) {
  ...
  if (mouse.hasPiece && !isPromoting) {
    if (
      (clickedSquare.piece == null ||
        clickedSquare.piece.team != mouse.piece.team) &&
      mouse.piece.canMoveTo(indices[0], indices[1])
    ) {
      if (
        indices[0] == mouse.piece.startingX &&
        indices[1] == mouse.piece.startingY &&
      ...
      if (
        indices[1] == 0 &&
        clickedSquare.piece.type == 'pawn' &&
        clickedSquare.piece.hasMoved
      ) {
...
function findClickedSquareIndices(currentX, currentY) {
  ...
  var columnClicked = 0;
  for (
    var i = leftEdgeOfBoard + boardWidth / 16;
    i < rightEdgeOfBoard;
    i += boardWidth / 8
  ) {
  ...
  var rowClicked = 0;
  for (
    var i = topOfBoard + boardWidth / 16;
    i < bottomOfBoard;
    i += boardWidth / 8
  ) {
...
function drawBoard() {
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      ...
      context.fillRect(
        (i * BOARD_WIDTH) / 4 + BOARD_WIDTH / 8,
        (j * BOARD_WIDTH) / 4 + BOARD_WIDTH / 8,
        BOARD_WIDTH / 8,
        BOARD_WIDTH / 8
      );
    }
